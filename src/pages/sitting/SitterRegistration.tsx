...
export default function SitterRegistration() {
  ...
  useEffect(() => {
    const resetCompletedRegistration = () => {
      if (sessionStorage.getItem('club_nanny_sitting_registration_completed') === 'sitter') {
        clearRegistrationData('sitterRegistrationData');
        clearRegistrationData('sitterRegistrationCheckout');
      ...
      <main className={`flex-1 ${runningAsApp ? 'pt-6' : 'pt-24'} pb-16 px-4`}>
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8">
            <form onSubmit={submitApplication} autoComplete="off">
              {/* Step 1: Account */}
              {step === 1 && (
