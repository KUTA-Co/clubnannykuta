...
export function SitterDashboardLayout() {
  const location = useLocation();
  const navigate = useNavigate();
  const goBackInApp = useAppBackNavigation("/sitting/sitter");
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6]">
      ...
      <aside className="hidden lg:block w-64 h-screen p-3 fixed top-0 left-0 z-50">
        <div className="rounded-2xl h-full flex flex-col overflow-hidden" style={{ backgroundColor: '#C77DA3' }}>
          {/* Logo */}
          ...
          <div className="p-4">
            <Link
              to="/for-sitters"
              className="flex items-center gap-3 px-4 py-3 rounded-xl text-white/60 hover:bg-white/10 hover:text-white transition-all"
            >
          ...
          <div className="p-4 border-t border-white/20">
            <div className="px-4 py-3 mb-2">
              <p className="text-sm font-medium text-white truncate">
                {user?.firstName || user?.email || "Sitter"}
              </p>
              <p className="text-xs text-white/60">Sitter Account</p>
            </div>
      ...
      <div className="lg:hidden min-h-screen flex flex-col bg-white">
        ...
        <header className="px-4 pt-3 pb-3 bg-white border-b border-gray-100">
          <div className="flex items-center justify-between">
            ...
            >
              <ArrowLeft className="w-4 h-4" style={{ color: '#C77DA3' }} />
            </button>
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{ backgroundColor: active ? '#E8A0BF' : 'transparent' }}
                  >
                    <Icon
                      className="w-5 h-5"
                  ...
                  <span
                    ...
                    style={{ color: active ? '#E8A0BF' : '#9CA3AF' }}
                  >
                    {item.label}
                  </span>
                </Link>
...
export default SitterDashboardLayout;
